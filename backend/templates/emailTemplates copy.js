...
const orderConfirmationEmailTemplate = (order, user, reviewLinks = []) => {
  ...
  if (reviewLinks && reviewLinks.length > 0) {
    const linksHtml = reviewLinks.map(link => 
      '<div style="margin: 10px 0;">' +
        '<a href="' + link.url + '" ' +
           'style="background-color: #000000; color: #ffffff; padding: 12px 20px; text-decoration: none; border-radius: 5px; display: inline-block; font-size: 14px;">' +
          'Donner mon avis sur ' + link.productName +
        '</a>' +
      '</div>'
    ).join('');
    ...
    reviewLinksHtml = 
      ...
      '<div style="background-color: #f5f5f5; padding: 20px; border-radius: 5px;">' +
        '<p style="color: #666666; font-size: 14px; line-height: 1.6; margin: 0 0 15px 0;">' +
          'Votre avis compte ! Cliquez sur les liens ci-dessous pour laisser un <strong>avis vérifié</strong> sur vos produits :' +
        '</p>' +
        linksHtml +
